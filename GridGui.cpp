...
void Grid::getMove(int &startX, int &startY, int &endX, int &endY) const
{//get move from mouse click
	Point p = cwin.get_mouse("From") ;
	startY = Y_SHIFT - int(p.get_y()+0.5); //round and shift
	startX = int(p.get_x()+0.5 - X_SHIFT); //round and shift
	
	p = cwin.get_mouse( "To") ;
	endY = Y_SHIFT - int(p.get_y()+0.5); //round and shift
	endX = int(p.get_x()+0.5 - X_SHIFT); //round and shift		
}
